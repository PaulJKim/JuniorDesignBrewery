	...
	return databaseReference.once("value").then((metadata) => {
		if (metadata.val().viewable) {
			...
			return databaseReference.once("value").then((review) => {
				...
				merged.revId = reviewId;
				return Promise.all([Promise.resolve(merged), getReviewPicture(reviewId, merged.userId)]);
			}).then(([review, image]) => {
				review.image = image
...
export function writeReview(breweryId, reviewData) {
	...
	return databaseReference.push().then((reviewReference) => {
		...
		var updates = {};
		updates["Reviews/" + reviewId + "/metadata/breweryId"] = breweryId;
		updates["Reviews/" + reviewId + "/metadata/userId"] = uid;
		updates["Reviews/" + reviewId + "/metadata/viewable"] = true;
...
/*
For the review pictures, we include the userid as well as the review id in the
path so we can allow only the review writer to change a photo
*/
export function setReviewPicture(pictureURI, reviewId) {
	const ourUid = firebaseApp.auth().currentUser.uid;
	if (pictureURI == null) {
		return Promise.resolve();
	}
	var storageReference = firebaseApp.storage().ref("Reviews/" + ourUid + '/' + reviewId + "/picture0");
	fetch(pictureURI).then((picture) => {
		return picture.blob()
	}).then((blob) => {
		return storageReference.put(blob)
...
export function getReviewPicture(reviewId, userId) {
	var storageReference = firebaseApp.storage().ref("Reviews/" + userId + '/' +  reviewId + "/picture0");
	return storageReference.getDownloadURL().catch(() => {
		return Promise.resolve(null);
	});
}
