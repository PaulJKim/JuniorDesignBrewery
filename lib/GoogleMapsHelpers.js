...
export function getBreweries(lat, lng, radius) {
  ...
  return fetch(url).then((response) => {
    ...
    promises = responseJSON.results.map((breweryResponse) => {
      return Promise.all([getPlaceDetails(breweryResponse.place_id), getOverallAverage(breweryResponse.place_id)]).then(([brewery, rating]) => {
